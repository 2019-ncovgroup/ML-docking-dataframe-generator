...
# Canonical SMILES translated from the ena+db.smi
"""
can = pd.read_csv(datapath/'original'/'ena+db.can',
                  header=None, names=['smiles', 'name'], sep='\t')
can['smiles'] = can['smiles'].map(lambda x: x.strip())
print_fn(can.shape);
print_fn('Total duplicates (do not remove): {}'.format( sum(can.duplicated()) ))
print_fn(can[:2]);
"""
...
cnt0 = can.shape[0]; print('\nCount: ', cnt0)
can = can.drop_duplicates().reset_index(drop=True)
cnt1 = can.shape[0]; print('Count: ', cnt1); print('Dropped duplicates: ', cnt0-cnt1)
"""
...
cnt0 = can.shape[0]; print('\nCount: ', cnt0)
can = can.drop_duplicates(subset=['smiles']).reset_index(drop=True)
cnt1 = can.shape[0]; print('Count: ', cnt1); print('Dropped duplicates: ', cnt0-cnt1)
"""
...
smi_vec = canon_df(smi_vec, smi_name='smiles')
smi['smiles'] = smi_vec
...
smi.to_csv(out_fpath, index=False)
