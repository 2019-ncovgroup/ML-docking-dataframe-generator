...
def smile_to_fps_single(smi, radius=2, nbits=2048):
...
def smiles_to_fps(df, radius=2, nbits=2048, smi_name='smiles', n_jobs=32):
    """ Canonicalize a smiles vector and generate df of FPs. """
    # t0 = time()
    res = Parallel(n_jobs=n_jobs, verbose=1)(
            delayed(smile_to_fps_single)(smi, radius=radius) for smi in df[smi_name].tolist())
